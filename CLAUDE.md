...
3. Wait for CI to finish: `gh pr checks <number> --watch`. The repo's checks (`.github/workflows/ci.yml`: typecheck, Convex typecheck, lint, format check, test, build) plus the Greptile review bot both must pass. If anything fails, fix it, push again, and re-watch.
